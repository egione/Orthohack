...
*/
/*
Orthohack Custom Fixed-Point-X8 Transform
*/
...
void
orthocustom_x8_init(CUSTOM monomial_x8_list_base[MONOMIAL_COUNT*8],CUSTOM sample_list_base[SAMPLE_IDX_MAX*8]){
/*
Prepare for a custom fixed-point-X8 transform.

In:

  *sample_list_base is readable for (SAMPLE_IDX_MAX*8) (CUSTOM)s. Successive sets of SAMPLE_IDX_MAX (CUSTOM)s belong to successive transform domains. The first sample should be actual sample data, unlike *monomial_x8_list_base, whose first item is always set to 0.

  *monomial_x8_list_base is writable for (MONOMIAL_COUNT*8) (CUSTOM)s.

Out:

  *monomial_x8_list_base is formatted so as to satisfy the assumptions of a custom fixed-point-X8 transform. Namely, the first (SAMPLE_IDX_MAX+1) sets of 8 monomials are initialized to: 8 0s, followed by (SAMPLE_IDX_MAX*8) samples from *sample_list_base, such that successive monomials correspond to the successive transforms, modulo 8.
*/
  CUSTOM custom;
  u64 monomial_idx;
  u64 sample_idx;
  ...
  do{
    for(thread_idx=0;thread_idx<=7;thread_idx++){
      custom=sample_list_base[sample_idx];
      sample_idx+=SAMPLE_IDX_MAX;
      monomial_x8_list_base[monomial_idx]=custom;
      monomial_idx++;
    }
...
void
orthocustom_x8_end(CUSTOM coeff_list_base[COEFF_COUNT*8],CUSTOM coeff_x8_list_base[COEFF_COUNT*8]){
/*
Issue the results of a custom fixed-point-X8 transform.

In:

  *coeff_x8_list_base is the output of orthocustom_x8_transform().

  *coeff_list_base is writable for (COEFF_COUNT*8) (CUSTOM)s. 

Out:

  *coeff_list_base contains 8 successive sets of COEFF_COUNT (CUSTOM)s, each set corresponding to the same set in orthocustom_x8_init():In:sample_list_base.
*/
  u64 coeff_idx;
  u64 coeff_x8_idx;
  CUSTOM custom;
  ...
  do{
    for(thread_idx=0;thread_idx<=7;thread_idx++){
      custom=coeff_x8_list_base[coeff_x8_idx];
      coeff_x8_idx++;
      coeff_list_base[coeff_idx]=custom;
...
void
orthocustom_x8_transform(CUSTOM coeff_x8_list_base[COEFF_COUNT*8],CUSTOM monomial_x8_list_base[MONOMIAL_COUNT*8]){
/*
Perform the custom fixed-point-X8 transform optimized by pre and post at the precision defined by PRECISION_CUSTOM in orthohack.h, which in turn results in the definition of a CUSTOM by orthocustom.h.

In:

  *coeff_x8_list_base is writable for (COEFF_COUNT*8) (CUSTOM)s.

  *monomial_x8_list_base is orthofloat_x8_init():Out:monomial_x8_list_base.

Out:

  *coeff_x8_list_base contains the coefficients, expressed as (CUSTOM)s, and arranged in the order corresponding to the ascending filenames which were originally fed into pre, such that each 8 successive coefficients correspond to successive transforms.

  *monomial_x8_list_base is unchanged where it was defined on input (the first ((SAMPLE_IDX_MAX+1)*8) (CUSTOM)s). Otherwise undefined.
*/
  u64 alias_count;
  CUSTOM coeff;
  u64 coeff_idx;
